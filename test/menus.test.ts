...
// code - a menu object with methods, items shown, greyed out and chosen by
// functions, a list menu with a filter and a listener of its own.
...
describe("an items menu", () => {
	test("visible leaves an item out, enabled greys it out with its message, onSelect runs", async () => {
		...
		expect(menus.screen(alice)!.text).toContain("\\d[1] Armor\\y (full)");
...
describe("a list menu", () => {
	test("a row per player the filter keeps; the item gets the row's player as its target", async () => {
		...
		expect(text).toContain("\\y[1]\\w Bob");
